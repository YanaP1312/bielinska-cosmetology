      ...
      {links.map(({ href, label }) => {
         ...
         return(
          <Link
            key={label}
            href={href}
            className={`text-xl text-center tracking-wider font-medium ${isActive ? "text-additional text-[26px] font-vibes" : "text-primary"} hover-interaction hover:text-additional`}
            onClick={onClose}
          >
            {label}
          </Link>)
