      ...
      {links.map(({ href, label }) => {
         ...
         return(
          <Link
            key={label}
            href={href}
            className={`text-xl text-center tracking-wider font-medium ${isActive ? "text-additional font-accent  text-3xl" : "text-primary"} hover-interaction hover:text-additional`}
            onClick={onClose}
          >
            {label}
          </Link>)
